...
def _get_secoes(tipo, modalidade=None, tipo_objeto=None):
    """
    Retorna lista de SecaoArtefato ativas para o tipo, filtradas por modalidade
    e por tipo de objeto (Bens/Serviços/Híbrido — só relevante para TR).
...
def gerar_pdf_tr(tr) -> bytes:
    ...
    # Bloco de identificação fixo — metadados de controle do Webber, sem
    # sobreposição com a seção "1. Condições Gerais" (cond_gerais, tipo de
    # objeto/delegação/SRP), que já é gerada pelo loop genérico abaixo.
    e.append(_secao('Identificação', estilos))
    ...
    # acima (_renderizar_secoes_tr, seções seedadas em seed_secoes_tr.py) — não
    # reimprimir aqui para não duplicar o mesmo conteúdo com título diferente.
