...
export const pageHelp = {
  titulo: 'Tipos de Fonte',
  descricao: 'Catálogo global de classificação de Fontes de Recurso, usado no cadastro de Fontes em Configurações → Fontes de Recurso.',
}
// ──────────────────────────────────────────────────────────────────────────────

export default function TipoFonteAdmin() {
  ...
  const load = () => {
    setLoading(true)
    api.get('/orcamento/tipo-fonte/', { params: { inativos: true, page_size: 200 } })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        await api.patch(`/orcamento/tipo-fonte/${editId}/`, payload)
        setMsg({ type: 'success', text: 'Tipo de fonte atualizado.' })
      } else {
        await api.post('/orcamento/tipo-fonte/', payload)
        setMsg({ type: 'success', text: 'Tipo de fonte criado.' })
      }
      reset(); load()
    } catch (err) {
  ...
  const handleInativar = async (t) => {
    if (!confirm(`Inativar o tipo "${t.descricao}"? Fontes de recurso já cadastradas com esse tipo continuam válidas.`)) return
    await api.delete(`/orcamento/tipo-fonte/${t.id}/`)
    load()
  }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-5 mb-6 space-y-4">
        ...
        <div>
          ...
          <input type="text" value={form.descricao} onChange={e => { setForm(p => ({ ...p, descricao: e.target.value })); setErrors(p => ({ ...p, descricao: undefined })) }}
            placeholder="Ex: Tesouro" className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.descricao ? 'border-red-400' : 'border-gray-300'}`} />
      ...
      {loading ? <LoadingSpinner /> : list.length === 0 ? (
        <EmptyState icon="document" title="Nenhum tipo cadastrado" description="Cadastre os tipos de fonte de recurso." />
      ) : (
