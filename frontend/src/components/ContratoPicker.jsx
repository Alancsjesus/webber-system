...
/**
 * Autocomplete de Contrato por número/objeto. onChange recebe o id do
 * contrato selecionado (ou null se limpo), mesmo padrão de FornecedorPicker.
 */
export default function ContratoPicker({ value, valueLabel, onChange, placeholder = 'Buscar por número ou objeto...' }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    api.get('/contratos/contrato/', { params: { search, page_size: 10 } })
  ...
  const selecionar = (contrato) => {
    setQuery(`${contrato.numero} — ${contrato.objeto?.slice(0, 50) || ''}`)
    setAberto(false)
    onChange(contrato.id, contrato)
  }
      ...
      {aberto && query && !value && (
        <div className="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-64 overflow-y-auto">
          {loading ? (
            ...
            <div className="px-3 py-2 text-xs text-gray-400">Nenhum contrato encontrado.</div>
          ) : (
            resultados.map(c => (
              <button key={c.id} type="button" onClick={() => selecionar(c)}
                ...
                <span className="block text-gray-800 truncate">{c.objeto}</span>
              </button>
            ))
          )}
        </div>
